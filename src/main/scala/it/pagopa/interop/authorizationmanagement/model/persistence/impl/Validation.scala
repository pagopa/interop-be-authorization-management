...
trait Validation {
  ...
  def validateKeys(keys: Seq[Key]): ValidatedNel[InvalidKey, Seq[ValidKey]] = keys.traverse(validateKey)

  private def validateKey(keySeed: Key): ValidatedNel[InvalidKey, ValidKey] = {
    val processedKey = for {
      jwk <- KeyConverter.fromBase64encodedPEM(keySeed.encodedPem)
      _   <- KeyConverter.publicKeyOnly(jwk)
    } yield jwk

    processedKey match {
      case Left(throwable) => InvalidKey(keySeed.encodedPem, throwable.getLocalizedMessage).invalidNel[ValidKey]
