...
import it.pagopa.interop.be.authorizationmanagement.common.system.shardingSettings
...
class KeyPersistentProjection(
  ...
  val projections: Seq[AtLeastOnceFlowProjection[Offset, EventEnvelope[Event]]] =
    (0 until settings.numberOfShards).map(i => projection(s"interop-be-authorization-management-persistence|$i"))
